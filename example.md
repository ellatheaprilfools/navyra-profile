...
navyra-profile analyse examples/synthetic_support.jsonl --html examples/support_report.html
```

```
prompts analysed          : 8,000
T1 exact repeats          : 7.8%   (already free via prefix caching)
T2 same-bucket semantic   : 14.0%  (engine-addressable today; upper-bound proxy)
T3 cross-bucket semantic  : 4.3%   (future-addressable)
...
T2 at 14.0% is the number worth acting on. This is reported as an upper bound: the true addressable rate is gated inside a matching engine, not measured directly here.
...
```
Quick one — what's the status of order 85636?
Quick one — what's the status of order 80635?
```
